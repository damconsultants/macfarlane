...
class AutoAddFromMagento
{
	 ...
	 * @param \DamConsultants\Macfarlane\Model\BynderAutoReplaceDataFactory $bynderAutoReplaceData
    ...
    public function __construct(
		...
		\DamConsultants\Macfarlane\Model\BynderAutoReplaceDataFactory $bynderAutoReplaceData,
     ...
     * Bynder API response, routed by each item's own "type" field. Nothing
     * from a previous cron run is read or merged in here.
     *
    ...
    /**
     * Writes the freshly built media data onto the product. This always
     * REPLACES bynder_multi_img / bynder_document outright with what this
     * run's API response contained - it never merges with, or reads, the
     * value that was stored before this run.
     *
     * @param string $sku
    ...
    public function getUpdateImage($sku, $image_items, $video_items, $doc_items)
    {
        try {
            ...
            if (count($image_items) > 0 || count($video_items) > 0) {
                ...
                // Keep bynder_media_table in sync with exactly what this
                // run returned - drop rows for ids that dropped out, add
                // rows for ids that are new.
            ...
            if (count($updated_values) > 0) {
                $updated_values['bynder_auto_replace'] = 1;
